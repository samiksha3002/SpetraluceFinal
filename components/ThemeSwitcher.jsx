...
import { useState, useEffect } from 'react'; // We still need these
...
export const ThemeSwitcher = ({ isScrolled }) => {
  ...
  // --- THIS IS THE FIX ---
  // We can't return null or a different component on the server.
  // We'll render the button, but make it 'invisible' and 'disabled' 
  // until the component is "mounted" on the client. This ensures 
  // the server and client HTML match.
  if (!mounted) {
    return (
      <button
        className="flex items-center gap-2 text-base font-medium transition-colors whitespace-nowrap
  ...
  // This is the dynamic color class from our Header
  const linkColorClass = isScrolled
    ? "text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white" // Scrolled
    : "text-gray-300 hover:text-white"; // Top-of-page (on hero)
